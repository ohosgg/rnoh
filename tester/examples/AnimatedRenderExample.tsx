...
export function AnimatedRenderExample() {
  const translateXYalue = React.useRef(new Animated.ValueXY({ x: 0, y: 0 })).current;
  ...
  return (
    <View style={[styles.mainBlock]}>
      <Animated.View
        style={[
          styles.viewBlock, { marginBottom: 100 },
          animated
      ...
      />
      <Text>{num}</Text>
      <Button onPress={() => { onPress.reset(); onPress.start() }} title="Start" />
      ...
      <Button disabled onPress={() => setAnimated(!animated)} title={`Toggle animated (${animated})`} />
    </View>
  )
}
