...
export const StyleSheetTest = () => {
  return (
    <TestSuite name="Stylesheet">
      <TestCase itShould="see if StyleSheet.create returns reference to an object with the correct props"
        fn={({ expect }) => {
          expect(styles.test).to.be.eql(testStyle)
        }} />
      <TestCase itShould="compose styles correctly"
        fn={({ expect }) => {
      ...
      <TestCase itShould="flatten styles correctly"
        fn={({ expect }) => {
          expect(StyleSheet.flatten([testStyle, testStyle2])).to.be.eql(flattenedStyle)
        }} />
      <TestCase itShould="return a sensible hairlineWidth value"
        fn={({ expect }) => {
