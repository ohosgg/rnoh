...
export function LinkingTest() {
  return (
    <TestSuite name="Linking (Stub)">
      ...
      />
      <TestCase
        itShould="open phone dialing on press"
        >
        <Button onPress={() => Linking.openURL('tel:1234567890')} label="Call 1234567890" />
      </TestCase>
      <TestCase
        itShould="open sms app (sms app doesn't accept parameters)"
        skip
        >
        <Button onPress={() => Linking.openURL('sms:1234567890')} label="Send SMS to 1234567890" />
      </TestCase>
      <TestCase
        itShould="open web page on press"
        >
        <Button onPress={() => Linking.openURL('https://reactnative.dev/docs/linking')} label="Open Linking Docs" />
      </TestCase>
      <TestCase
        itShould="open (http) web page on press"
        >
        <Button onPress={() => Linking.openURL('http://info.cern.ch/')} label="Open an old webpage" />
      </TestCase>
      <TestCase
        itShould="open application settings"
        >
