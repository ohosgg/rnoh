...
import { Keyboard, StyleSheet, Text, TextInput, KeyboardEvent, Dimensions } from 'react-native';
  ...
  return (
    <TestSuite name="Keyboard">
      ...
      <TestCase
        modal
        skip
        initialState={{ keyboardDidHide: false, keyboardDidShow: true, keyboardEvent: {} as KeyboardEvent }}
        itShould="not open the keyboard after opening the modal"
        arrange={({ setState, reset }) => { return (<KeyboardEventsView listenToShow checkIfKeyboardIsVisible onSetState={setState} reset={reset} />) }}
        assert={({ expect, state }) => {
      ...
      <TestCase
        modal
        initialState={{ keyboardDidHide: false, keyboardDidShow: false, keyboardEvent: {} as KeyboardEvent }}
        itShould="pass after opening the keyboard"
        arrange={({ setState, reset }) => { return (<KeyboardEventsView listenToShow dismissOnReset onSetState={setState} reset={reset} />) }}
        assert={({ expect, state }) => {
          expect(state.keyboardDidShow).to.be.true;
          expect(state.keyboardDidHide).to.be.false;
          expect(state.keyboardEvent.endCoordinates.height).to.be.greaterThan(0)
          expect(state.keyboardEvent.endCoordinates.width).to.be.greaterThan(0)
          expect(state.keyboardEvent.endCoordinates.screenY).to.be.lessThan(Dimensions.get('window').height)
      ...
      <TestCase
        ...
        arrange={({ setState, reset }) => { return (<KeyboardEventsView listenToHide showDismissButton onSetState={setState} reset={reset} />) }}
        assert={({ expect, state }) => {
          expect(state.keyboardDidHide).to.be.true;
          expect(state.keyboardDidShow).to.be.false;
          expect(state.keyboardEvent.endCoordinates.height).to.be.equal(0)
          expect(state.keyboardEvent.endCoordinates.width).to.be.greaterThan(0)
          expect(state.keyboardEvent.endCoordinates.screenY).to.be.closeTo(Dimensions.get('window').height, 10)
          expect(state.keyboardEvent.endCoordinates.screenX).to.be.equal(0)
        }}
      />
...
function KeyboardEventsView(props: {
  ...
  useEffect(() => {
    ...
    if (props.listenToShow) {
      showSubscription = Keyboard.addListener(
        'keyboardDidShow',
        (event) => {
          props.onSetState(prev => ({ ...prev, keyboardEvent: event, keyboardDidShow: true }))
        },
      );
    }
    if (props.listenToHide) {
      hideSubscription = Keyboard.addListener(
        'keyboardDidHide',
        (event) => {
          props.onSetState(prev => ({ ...prev, keyboardEvent: event, keyboardDidHide: true, }))
        },
      );
    }
  ...
  return (
    <>
      ...
      {props.showDismissButton && <Button label={"dismiss keyboard"} onPress={() => Keyboard.dismiss()} />}
